...
Prim = R6::R6Class("Prim", inherit = RegDescMethod,
  ...
  private = list(
    ...
    run = function() {
      ...
      private$obsdata = rbind(private$x_interest,
                              private$obsdata[, private$predictor$data$feature.names, with = FALSE]
                              )
      private$.calls_fhat = private$.calls_fhat + nrow(private$obsdata)
      private$obsdata = private$obsdata[, positive := predict_range(private$predictor,
      ...
      if (is.null(private$box_largest)) {
        private$box_largest = get_max_box(private$x_interest,
                                          ...
                                          desired_range = private$desired_range,
                                          resolution = 500L)
                               # <FIXME:> is this a good parameter value??
      }
          ...
          if (box_new$is_categ[[j]]) {
            ...
            for (cat in setdiff(box_new$levels[[j]], private$x_interest[[j]])) {
              subbox = update_box(current_box = box_new,
                                  ...
                                  lower = NULL,
                                  upper = NULL,
                                  val = setdiff(box_new$levels[[j]], cat),
                                  complement = FALSE)

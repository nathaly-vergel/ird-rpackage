...
get_connected_samples = function(predictor,
                                 x_interest,
                                 desired_range,
                                 fixed_features = NULL,
                                 num_sampled_points = 10L,
                                 convexset = FALSE,
                                 resolution = 50L,
                                 max_permutations = 100L,
                                 seed = 301122L) {
  ...
  if (nrow(sampleddata) > nrow(connected) && !convexset) {
    ...
    for (row in sample(seq_len(nrow(remaining)), size = nrow(remaining))) {
      ...
      connectid = nrow(get_path(x_interest = remaining[row, ],
                                sampleddata = connected,
                                predictor = predictor,
...
get_path = function(x_interest,
  ...
  if (length(ordfeat) > 0 & nrow(sampleddata) > 0) {
    ...
    check_ord_path = function(row, perm, x_interest) {
      if (isTRUE(all.equal(row[, (perm), with = FALSE], x_interest[, (perm), with = FALSE], check.attributes = FALSE))) {
        return(TRUE)
      }
